...
- **Emulator SQL limitations**: no `CREATE DATABASE`, no three-part names (`DB.SCHEMA.TABLE`), no `PARSE_JSON()`, no explicit `COMMIT`, no `information_schema` catalog queries (`dbt docs generate` fails at catalog step). Use unqualified table names (session context is `BANK_DB.RAW`). Use `INSERT INTO t (col) VALUES ('...')` for JSON rows.
