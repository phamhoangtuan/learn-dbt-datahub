...
```bash
make infra-up    # Snowflake Emulator on :8082
make datahub-up  # DataHub on :8080 (takes ~60s to initialize)
make run-full    # generate → load → transform → datahub-ingest
# Then open http://localhost:9002 to explore lineage
```

> Emulator runs on port **8082**, DataHub GMS on port **8080** — no conflict, both run simultaneously.
