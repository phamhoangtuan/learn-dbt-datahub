...
make run-full    # emulator up → pipeline → emulator down → datahub-ingest
...
> `make run-full` stops the emulator before ingesting. Start DataHub **before** running `make run-full`.
