...
- [x] **Snowflake Emulator**: Running via Docker on `localhost:8082` (`ghcr.io/nnnkkk7/snowflake-emulator`).
...
```bash
make infra-up    # Start Snowflake Emulator on port 8082
make run         # generate → load → transform
```

### Run everything including governance (Phase 5–6)
```bash
# Terminal 1: start DataHub first (takes ~60s to initialize)
make datahub-up

# Terminal 2: once DataHub is ready, run the full pipeline + metadata ingest
make run-full    # emulator(:8082) up → pipeline → emulator down → datahub-ingest(:8080)
# Then open http://localhost:9002 to explore lineage
```

> Emulator runs on port **8082**, DataHub GMS on port **8080** — no conflict, both can run simultaneously.
